 ...
 * @Author: your name
 * @Date: 2019-08-16 09:59:44
 * @LastEditTime: 2019-08-16 11:02:04
...
int read_conf(char* filename, tk_conf_t *conf)
{
    ...
    int buff_len = BUFLEN;
    char* curr_pos = buff;
    char* delime_pos = NULL;
    int i = 0;
    int pos = 0;
    int line_len = 0;
    
    while(fgets(curr_pos, buff_len-pos, fp)) {
        delime_pos = strstr(curr_pos, DELIM);
        if (!delime_pos)
            return TK_CONF_ERROR;
        ...
        if (strncmp("root", curr_pos, 4) == 0) {
            delime_pos = delime_pos + 1;
            while(*delime_pos != '#') {
                conf->root[i++] = *delime_pos;
                ++delime_pos;
            }
        }

        if (strncmp("port", curr_pos, 4) == 0)
            conf->port = atoi(delime_pos + 1);

        if (strncmp("thread_num", curr_pos, 9) == 0)
            conf->thread_num = atoi(delime_pos + 1);
...
void accept_connection(int listen_fd, int epoll_fd, char* path)
{
    ...
    // 设为非阻塞模式
    int rc = make_socket_non_blocking(accept_fd);//判断？
    ...
    tk_epoll_add(epoll_fd, accept_fd, 
                 request, 
